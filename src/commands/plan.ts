...
export async function cmdPlan(args: string[]): Promise<void> {
  ...
  switch (sub) {
    case 'enter': {
      ...
      await enterPlanMode(teamName, teammate, taskId);
    ...
    case 'submit': {
      ...
      const req = await submitPlanForApproval(teamName, teammate, taskId, plan);
    ...
    case 'review': {
      ...
      const result = await reviewPlan(teamName, requestId, decision as 'approved' | 'rejected', flags['feedback']);
    ...
    case 'history': {
      ...
      const history = await getPlanHistory(teamName, teammate, taskId);
    ...
    case 'criteria': {
      if (flags['set']) {
        await setApprovalCriteria(teamName, { description: flags['set'] });
