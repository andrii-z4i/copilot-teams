...
function defaultSpawnCommandBuilder(
  ...
  const teammateContext = [
    ...
    `- Use submit_report to save your detailed findings/report for each task (team_name: "${teamName}")`,
    `- Use claim_file before editing files to avoid conflicts (team_name: "${teamName}")`,
    '',
    'CRITICAL REQUIREMENTS — YOU MUST DO ALL OF THESE BEFORE FINISHING:',
    '1. For each task you complete, call update_task to mark it completed',
    `2. For each task, call submit_report with your DETAILED findings (teammate_name: "${options.name}", team_name: "${teamName}")`,
    '   Include: what you found, severity ratings, affected files, and recommendations',
    '3. After ALL work is done, call send_message to "lead" with a brief summary',
    'The submit_report tool is how the lead retrieves your work. Do NOT skip it.',
    '',
    'YOUR TASK:',
  ].join('\n');
